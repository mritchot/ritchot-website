...
1. **`suminagashi`** (homepage only, `client:idle`): canvas ink-marbling hero. A static frozen-marble SVG poster paints first and is the permanent state for no-JS, reduced-motion, and island failure. Decorative (`aria-hidden`); simulation pauses when hidden.
