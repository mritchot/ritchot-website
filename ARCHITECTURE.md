...
1. **`suminagashi`** (homepage only, `client:idle`): WebGL ink-dissolution hero (D21) — a stable-fluids dye field, continuously alive with autonomous multi-hue token-ink pours; pigment compositing keeps it matte ink, never neon. A static frozen-marble SVG poster paints first and is the permanent state for no-JS, reduced motion, island failure, and lost GL contexts. Decorative (`aria-hidden`); simulation pauses when hidden or off-view.
2. **`pretext-justify`** (writing pages, `client:visible`): upgrades CSS justification to Knuth–Plass optimal line breaking via [pretext](https://github.com/chenglou/pretext). Requires `Intl.Segmenter`; skips silently otherwise. Adoption is gated on measured size and zero visible reflow — the CSS baseline is a permanent acceptable fallback.
